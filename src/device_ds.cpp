...
namespace audiere {
  ...
  DSAudioDevice::create(const ParameterList& parameters) {
    ...
    min_buffer_length = min_buffer_length > 1 ? min_buffer_length : 1; // wdh: std::max(1, min_buffer_length);
    ...
    int channel_count, int sample_rate, SampleFormat sample_format)
  {
    ...
    const int buffer_frame_count = m_min_buffer_length > frame_count ? m_min_buffer_length : frame_count; // wdh std::max(m_min_buffer_length, frame_count);
